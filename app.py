...
with st.sidebar:
    ...
    if not is_admin:
        pwd = st.text_input("Code Administrateur", type="password", help="Saisissez le code pour activer les droits de modification directe et de validation.")
...
with st.expander("❓ Guide complet : Comment importer vos modifications ?"):
    ...
    **Format du fichier CSV attendu :**
    - **Colonne 1** : {C_INSEE} (ex: 75001)
    - **Colonne 2** : Le nom souhaité pour le comité.
    - *Note : Seul l'ordre des colonnes compte, les entêtes de votre fichier peuvent différer.*

    **Règles selon votre profil :**
    1. **Utilisateur (Standard)** : Votre e-mail est obligatoire. Le fichier remplit la colonne *{C_NOUVEAU_NOM}* pour soumission.
    2. **Administrateur** : L'e-mail n'est pas requis. Le fichier met à jour **directement** la colonne *{C_NOM_OFFICIEL}* et horodate la modification.
    """)

col_mail, col_file = st.columns([1, 2])
with col_mail:
    if not is_admin:
        contact_mail = st.text_input("📧 Votre e-mail de contact :", placeholder="prenom.nom@domaine.fr", help="Obligatoire pour tracer l'origine de la demande de modification.")
        email_valid = bool(re.match(r"^[\w\.-]+@[\w\.-]+\.\w+$", contact_mail)) if contact_mail else False
    else:
        st.info("✅ **Identifié comme Admin**")
        st.caption("Les modifications seront appliquées directement sans validation intermédiaire.")
        email_valid = True

with col_file:
    up_file = st.file_uploader("Charger le fichier CSV de mise à jour", type="csv", help="Sélectionnez un fichier CSV encodé en UTF-8.")
...
# Configuration des colonnes (Entêtes \n et Masquage IDs)
view_config = {
...
if is_admin:
    ...
    f_edit = st.checkbox("Éditer uniquement la sélection filtrée", value=True, help="Décochez pour voir et modifier toute la base.")
    ...
    with c1:
        if st.button("💾 Sauvegarder les saisies manuelles", use_container_width=True, help="Enregistre les modifications effectuées directement dans les cellules du tableau."):
            st.session_state.df_main.update(edited_df)
            st.success("Modifications manuelles enregistrées.")
            st.rerun()
    with c2:
        if st.button("🗑️ Purger les propositions", use_container_width=True, help="Efface les demandes de nouveaux noms et les contacts pour la sélection filtrée."):
            st.session_state.df_main.loc[df_display.index, [C_NOUVEAU_NOM, C_CONTACT, C_DATE_DEMANDE]] = ""
            st.rerun()
    with c3:
        if st.button("✅ Valider et Écraser les noms", use_container_width=True, type="primary", help="Approuve les propositions : le Nouveau Nom devient le Nom Officiel."):
            today = datetime.now().strftime("%d/%m/%Y")
...
sel_cols = st.multiselect("Sélectionnez les colonnes à exporter :", options=st.session_state.df_main.columns.tolist(), default=[C_INSEE, C_NOM_OFFICIEL], help="Par défaut, l'export contient les colonnes minimales requises.")

if sel_cols:
    csv_data = df_display[sel_cols].to_csv(index=False, sep=';', encoding='utf-8-sig').encode('utf-8-sig')
    st.download_button(label=f"⬇️ Télécharger le CSV ({len(df_display)} lignes)", data=csv_data, file_name="referentiel_clpe_export.csv", mime="text/csv")
